...
✅ **Open Source**: No minified JavaScript - view source to audit the complete code  
...
The codebase is organized into separate files for easier auditing:

- **`index.html`** - HTML structure only (no inline scripts or styles)
- **`styles.css`** - All CSS styling
- **`crypto.js`** - Cryptographic operations (ECDH key exchange, HKDF derivation, AES-GCM encryption, TLV token encoding, key management).
